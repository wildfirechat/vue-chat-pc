...
class WfcImpl {
    ...
    init(args = []) {
        ...
        // }
        protoProxy.invoke('setAppName', 'config/cn.wildfirechat.pcclient');
    ...
    async sendMessageEx(message, toUsers = [], preparedCB, progressCB, successCB, failCB) {
        ...
        if (message.messageContent instanceof MediaMessageContent && (message.messageContent.localPath || message.messageContent.file) && !message.messageContent.remotePath && this.isSupportBigFilesUpload()) {
            let insertedMsg = this.insertMessage(message.conversation, message.messageContent, MessageStatus.Sending, false, 0);
    ...
    async uploadMedia(fileName, data, mediaType, successCB, failCB, progressCB) {
        ...
        if (this.isTcpShortLink()) {
            const byteCharacters = atob(data);
            const byteArray = new Uint8Array(byteCharacters.length);
            for (let i = 0; i < byteCharacters.length; i++) {
                byteArray[i] = byteCharacters.charCodeAt(i);
            }
            let file = new Blob([new Uint8Array(byteArray, 0, byteArray.byteLength)]);
            this._uploadFile(null, file, progressCB, successCB, failCB);
